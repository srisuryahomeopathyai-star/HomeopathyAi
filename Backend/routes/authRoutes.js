...
router.put("/apikey", middleware, async (req, res) => {
  try {
    ...
    res.json({ message: "API key saved", geminiApiKey: user.geminiApiKey });
  } catch (err) {
    res.status(500).json({ message: "Server error" });
  }
});
...
// Delete API key
router.delete("/apikey", middleware, async (req, res) => {
  try {
    await User.findByIdAndUpdate(req.user.id, { $unset: { geminiApiKey: 1 } });
...
router.post("/login", async (req, res) => {
  try {
    ...
    // Send OTP email with provider fallback
    try {
      ...
      if (resendKey) {
        await axios.post(
          "https://api.resend.com/emails",
          {
            from: fromEmail,
            to: adminEmail,
            subject: "Untrusted device login",
            text: `OTP for ${user.email} is ${otp}. Valid for 5 minutes.`,
          },
          {
            headers: {
        ...
        await axios.post(
          "https://api.sendgrid.com/v3/mail/send",
          {
            ...
            from: { email: fromEmail },
            subject: "Untrusted device login",
            content: [{ type: "text/plain", value: `OTP for ${user.email} is ${otp}. Valid for 5 minutes.` }],
          },
          {
            headers: {
              Authorization: `Bearer ${sendgridKey}`,
        ...
        if (String(process.env.DISABLE_SMTP).toLowerCase() === "true") {
          throw new Error("SMTP disabled and no HTTP email provider configured");
        }
        const nodemailer = require("nodemailer");
        const transporter = nodemailer.createTransport(
          process.env.SMTP_SERVICE
      ...
      console.error("OTP Email Failed:", e.response?.data || e.message);
    }
