...
const CasesList = () => {
  ...
  const [userId, setUserId] = useState(null); // State for decoded user ID
  const casesPerPage = 5;

  // AI summary state
  const [aiSummary, setAiSummary] = useState("");
  const [loadingSummary, setLoadingSummary] = useState(false);
  const [loadingCases, setLoadingCases] = useState(true); // Set to true initially
  ...
  // --- 1. Fetch Data and Decode User ID ---
  useEffect(() => {
    let decodedUserId = null;
    if (token) {
      try {
        const decoded = jwtDecode(token);
        decodedUserId = decoded.id; // Capture ID for filtering
        setUserId(decoded.id); // Set ID in state
    ...
    const fetchData = async () => {
      ...
      try {
        // Fetch all cases (assuming the API returns all cases)
        const res = await axios.get(`${API_URL}/api/cases`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        const allCases = res.data;

        // Filter the cases to only include those created by the logged-in user
        // ASSUMPTION: Each case object has a 'user' property containing the creator's ID
        const userCases = allCases.filter((c) => c.userId === decodedUserId);

        setCases(userCases);
        setFilteredCases(userCases); // Initial view is the user's cases
      } catch (err) {
    ...
    fetchData();
  }, [token]); // Dependency on token ensures fetch runs on load/token change

  // --- 2. Local Search Filter Effect ---
  useEffect(() => {
    const term = searchTerm.toLowerCase();

    // Filter the 'cases' array (which already contains only the current user's cases)
    ...
    setCurrentPage(1);
  }, [searchTerm, cases]); // Dependency on 'cases' ensures re-filter when the main list is loaded/updated
  ...
  const handleSaveCase = async () => {
    try {
      // Add this line to verify prescription array
      console.log("Sending prescriptions:", selectedCase.prescription);
      const token = localStorage.getItem("token");
            ...
            {currentCases.map((c) => (
              <tr key={c._id}>
                ...
                <td>
                  ...
                  <button onClick={() => handleDeleteCase(c._id)}>
                    Delete
                  </button>
                </td>
              </tr>
        ...
        >
          ...
          {(viewCase.prescription || []).map((p, i) => (
            <div key={i} style={{ marginBottom: "15px", padding: "10px", border: "1px solid #eee" }}>
              ...
              <p>
                <strong>Potency:</strong> {p.potency || "Not available"}
              </p>
              <p>
